...
$nsh += "!define CLIENT_VOL0 `"$($vols[0].Name)`""
...
for ($i = 0; $i -lt $vols.Count; $i++) {
    ...
    $nsh += "  !insertmacro DL_VOLUME $i `"$($vols[$i].Name)`" `"$sha`""
}
$nsh += "!macroend"
...
if ($Publish) {
    ...
    if ($LASTEXITCODE -eq 0) {
        & $gh release upload $tag @($vols.FullName) --repo $Repo --clobber
    } else {
        & $gh release create $tag @($vols.FullName) --repo $Repo --title "Client base $Version" --notes $notes
    }
    ...
    git -C $RepoDir push
    Write-Host "`npublished $tag. Rebuild the installer to bundle the new volume list."
} else {
    Write-Host "`nnot published. Re-run with -Publish -Repo <owner/name>, or upload $out\*.7z.* yourself."
}
